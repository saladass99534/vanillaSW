...
export const ViewerRoom: React.FC<ViewerRoomProps> = ({ onBack }) => {
  ...
  // NEW: Video Time State & Subtitles
  const [currentTime, setCurrentTime] = useState(0);
  const [duration, setDuration] = useState(0);
  const [subtitleUrl, setSubtitleUrl] = useState<string | null>(null);
  const [movieTitle, setMovieTitle] = useState<string>(""); // NEW: Title State
  ...
  // Format Time Helper (Updated to show Hours)
            ...
            if (!peerRef.current) {
                ...
                p.on('data', (data) => {
                    try {
                        ...
                        // MOVIE TITLE SYNC (Assuming you broadcast this from host if you want, currently local state is tricky without explicit sync)
                        // If you want true sync, add a 'file_meta' broadcast type in HostRoom.
  ...
  useEffect(() => {
      const handleGlobalKeyDown = (e: KeyboardEvent) => { resetInputIdleTimer(); if ((isTheaterMode || isFullscreen) && !isInputFocused) { if (e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) { setShowControls(true); chatRef.current?.focusInput(); } } };
  ...
  const handleSendMessage = (text: string, type: 'text' | 'gif' = 'text') => {
      if (peerRef.current?.connected) {
          const msg = { id: Date.now().toString(), senderId: myUserId, senderName: username, text, timestamp: Date.now(), type };
    ...
    <div className="flex h-screen bg-[#111] text-gray-100 overflow-hidden font-sans">
      <div className="flex-1 flex flex-col relative bg-black min-w-0">
        <div ref={containerRef} className="flex-1 flex items-center justify-center relative bg-black group" onMouseMove={handleMouseMove} onClick={() => { if (!electronAvailable) { setShowControls(!showControls); } }}>
            ...
            {/* Top Controls */}
            <div onMouseEnter={clearControlsTimeout} onMouseLeave={resetControlsTimeout} onClick={(e) => e.stopPropagation()} className={`absolute top-0 right-0 z-20 p-4 transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}><Button size="sm" variant="danger" onClick={() => setShowExitConfirm(true)} className="rounded-full px-4">Leave</Button></div>
            ...
            <video ref={videoRef} className={`relative z-10 w-full h-full object-contain ${!hasStream ? 'hidden' : ''}`} autoPlay playsInline onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} onTimeUpdate={() => setCurrentTime(videoRef.current?.currentTime || 0)} onLoadedMetadata={() => setDuration(videoRef.current?.duration || 0)}>{subtitleUrl && <track key={subtitleUrl} label="English" kind="subtitles" src={subtitleUrl} default />}</video>
            ...
            <div onMouseEnter={clearControlsTimeout} onMouseLeave={resetControlsTimeout} onClick={(e) => e.stopPropagation()} className={`absolute bottom-8 left-1/2 -translate-x-1/2 z-50 transition-all ${controlsVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <div className="flex flex-col items-center gap-2">
                    ...
                    {duration > 0 && duration !== Infinity && ( // Only show if duration is valid and not live
                    ...
                    <div className="flex items-center gap-4 px-6 py-3 rounded-full bg-black/40 backdrop-blur-xl border border-white/10 shadow-2xl hover:bg-black/50 hover:scale-[1.02] transition-all">
                        ...
                        <div className="flex gap-2 items-center"><button onClick={() => handlePickerAction('start_picker')} disabled={(pickerStep !== 'idle' && pickerStep !== 'reveal')} className={`p-2 hover:bg-white/10 rounded-full transition-colors ${(pickerStep !== 'idle' && pickerStep !== 'reveal') ? 'text-gray-600 cursor-not-allowed' : `${activeTheme.primary}`}`} title="Suggest Movie"><Clapperboard size={20} /></button><button onClick={() => setShowNerdStats(!showNerdStats)} className={`p-2 hover:bg-white/10 rounded-full transition-colors ${showNerdStats ? `${activeTheme.primary}` : 'text-gray-400 hover:text-white'}`} title="Nerd Stats"><Activity size={20} /></button><button onClick={togglePiP} className="p-2 hover:bg-white/10 rounded-full text-gray-400 hover:text-white active:scale-95" title="Picture-in-Picture"><PictureInPicture size={20} /></button><button onClick={toggleTheaterMode} className={`p-2 hover:bg-white/10 rounded-full transition-colors ${isTheaterMode ? `${activeTheme.primary}` : 'text-gray-400 hover:text-white'}`} title="Toggle Theater Mode"><Tv size={20} /></button><button onClick={toggleFullscreen} className="p-2 hover:bg-white/10 rounded-full text-gray-400 hover:text-white transition-colors" title="Fullscreen">{isFullscreen ? <Minimize size={20} /> : <Maximize size={20} />}</button></div>
                    </div>
                </div>
            </div>
        </div>
      ...
      <div className={`bg-black/40 backdrop-blur-xl flex flex-col md:flex-none min-h-0 min-w-0 transition-all duration-500 ease-in-out rounded-l-3xl border-l shadow-2xl overflow-hidden ${sidebarCollapsed ? 'w-0 max-w-0 opacity-0 border-0' : 'w-80 opacity-100'} ${activeTheme.border} ${activeTheme.glow}`}>
        <div className={`min-w-[320px] h-full flex flex-col transition-transform duration-500 ease-in-out ${sidebarCollapsed ? 'translate-x-full' : 'translate-x-0'}`}>
          ...
          <div className="flex-1 relative min-h-0">{activeTab === 'chat' && <div className="absolute inset-0 flex flex-col"><Chat messages={messages} onSendMessage={handleSendMessage} onAddReaction={()=>{}} onHypeEmoji={handleHypeAction} onPickerAction={handlePickerAction} myId={myUserId} theme={activeTheme} onInputFocus={() => setIsInputFocused(true)} onInputBlur={() => setIsInputFocused(false)} onInputChange={resetInputIdleTimer} /></div>}{activeTab === 'members' && <div className="p-4 space-y-2 overflow-y-auto">{members.map(m => (<div key={m.id} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 hover:bg-white/10 transition-colors"><div className="flex items-center gap-3"><div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${m.isHost ? 'bg-yellow-400 text-black' : 'bg-white/10'}`}>{m.name[0]}</div><span className="text-sm font-medium text-gray-200">{m.name}</span></div>{m.isHost && <Crown size={14} className="text-yellow-500 drop-shadow-md" />}</div>))}</div>}</div>
        </div>
      </div>
    </div>
  );
